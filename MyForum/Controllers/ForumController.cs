    ...
    public class ForumController : Controller
    {
        ...
        private async Task InitializeAsync()
        {
            ...
            try
            {
                ...
                themesCount = _forumContext.Themes.Count();
                categoriesCount = _forumContext.Categories.Count();
            ...
            if (usersCount == 0)
            {
                Debug.WriteLine("Create admin user");
                User newUser = new User
                {
                    Email = adminEmail,
                    UserName = adminUsername,
                };
                IdentityResult creationResult = await _userManager.CreateAsync(newUser, adminPassword);
            ...
            admin = await _forumContext.Users.FirstOrDefaultAsync(i => i.Email == adminEmail);
            if (admin == null)
                throw new Exception("Failed to find admin user");


            Theme th = new Theme { Author = admin, CreatingTime = DateTime.Now };
            if (themesCount == 0)
            {
                await _forumContext.Themes.AddAsync(th);
                await _forumContext.SaveChangesAsync();
            }
            Debug.WriteLine($"New theme guid: {th.Id}");


            if(messagesCount == 0)
            {
                Message msg1 = new Message { Text = "Ребята, помогите, срочно нужна помощь!!!", Author = admin, CreatingTime = DateTime.Now, Theme = th };
                Message msg2 = new Message { Text = "Не отчаивайся, работяга, щас помогу тебе", Author = admin, CreatingTime = DateTime.Now, Theme = th };
                await _forumContext.Messages.AddRangeAsync(msg1, msg2);
                await _forumContext.SaveChangesAsync();

                th.Title = msg1.Text;
                th.Description = "Тут такая проблема возникла, в общем в силу обстоятельств и из-за нелепой случайности произошла проблема, которую мне непосильно решить, и я решил спросить помощи у вас...";
                _forumContext.Themes.Update(th);
                await _forumContext.SaveChangesAsync();
                ...
                ThemeSection themeSection1 = new ThemeSection { Section = sec1, Theme = th };
        ...
        public async Task<IActionResult> Index()
        {
            ...
            foreach (var m in model)
            {
                ...
                {
                    Message? lastMsg = _forumContext.Sections
                        .AsNoTracking()
                        .Include(s => s.Themes)
                        .ThenInclude(ts => ts.Theme)
                        .ThenInclude(t => t.Messages)
                    ...
                    if (lastMsg == null)
                        throw new Exception("Last message is null");

                    section.LastMessage = lastMsg.CreateLastMessage();

                    Section? sect = _forumContext.Sections
                        .AsNoTracking()
                        .Include(s => s.Themes)
                        .ThenInclude(ts => ts.Theme)
                        .ThenInclude(t => t.Messages)
                    ...
                    section.MessagesCount = msgsCount;
                }
            }
